...
#'  		\item  the resampled Fst as calculated from bootstrapping across simulations, with standard deviation and 95\% CI's,
 ...
 # for AFLP markers, i.e. dominant markers  
  ...
  for(i in 1:nsim) {
   ...
   #1. Fst simulation replicate; sample nloci from the neutral markers, with replacement
    ...
    if(AFLP==FALSE){ fst.repl <- fst.sample(fst.dat, nloci) }
...
#########################       wc.calc requisite functions      ###############################
#
# These functions are all taken directly from the hierfstat package by Jerome Goudet in order to allow
#	the modified Wier and Cockerham estimates function properly in wc.calc
#	See the hierfstat documentation for further explanations: http://cran.r-project.org/web/packages/hierfstat/index.html
...
allele.count <- function (data, diploid = TRUE){
    ...
    nbloc <- dim(data)[2] - 1
    fun <- function(x){ dum <- table(x, data[, 1]) }
...
pop.freq <- function(data, diploid = TRUE){
   	...
   	nbloc <- dim(data)[2] - 1
   	if (diploid) {
   	    data <- data.frame(data, dummy.loc = sample(101:999, 
   	        replace = TRUE, size = dim(data)[1]) * 1001)
   	    data <- getal(data)[, -2]
   	}else{
       	data <- data.frame(data, dummy.loc = sample(101:999, 
       	    replace = TRUE, size = dim(data)[1]))
   	}
   	freq <- function(x){
   	    dum <- table(x, data[, 1])
...
getal.b <- function(data){
   	x <- dim(data)
   	if (max(data[, 1], na.rm = TRUE) < 1e+06)	modulo = 1000
...
fst.sample <- function(obs, nloci) {
  ...
  # choose the loci to randomly sample:
